""" 
WORKS on Astro Pi Replay Tool! However wrong time estimate
"""
...
try:
    ...
    print("WARNING: picamzero not available")
...
try:
    ...
    print("WARNING: sense_hat not available")
...
try:
    ...
    print("WARNING: astro_pi_orbit not available")
    ISS_AVAILABLE = False
...
def calculate_robust_median(values):
    """
    Calculate median with outlier rejection using MAD (Median Absolute Deviation).
    
    Args:
        values: List of numbers
    
    Returns:
        float: Robust median value
    """
...
def main():
    ...
    if CAMERA_AVAILABLE:
        camera = Camera()
        print("✓ Camera initialized (picamzero)")
    else:
    ...
    if SENSEHAT_AVAILABLE:
        try:
            ...
            print("✗ SenseHAT initialization failed")
    ...
    try:
        ...
        print(f"✓ Capture complete: {len(image_files)} images")
    ...
    try:
        ...
        print(f"✓ Result written to result.txt: {result_string}")
    except Exception as e:
